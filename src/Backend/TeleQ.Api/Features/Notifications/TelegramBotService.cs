...
/// <summary>
/// Background service that hosts the Telegram Bot via long-polling.
/// Registers customer phone numbers and routes commands to the API logic.
/// </summary>
    ...
    IServiceScopeFactory scopeFactory) : BackgroundService
{
    ...
    private async Task HandleUpdateAsync(ITelegramBotClient client, Update update, CancellationToken ct)
    {
        ...
        {
            logger.LogError(ex, "Telegram bot update handling failed for update {UpdateId}", update.Id);

            var chatId = update.Message?.Chat.Id ?? update.CallbackQuery?.Message?.Chat.Id;
            if (chatId.HasValue)
            {
                await SafeSendMessageAsync(client, chatId.Value, ex is InvalidOperationException ? ex.Message : "Sorry, something went wrong. Please try again.", ct);
            }
    ...
    private async Task HandleCancelAsync(ITelegramBotClient client, long chatId, CancellationToken ct)
    {
        ...
        ClearContext(chatId);
        var message = await CancelActiveTicketAsync(customer.PhoneNumber, ct);
    ...
    private async Task<IList<Branch>> GetActiveBranchesAsync(CancellationToken ct)
    {
        await using var scope = scopeFactory.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<AppDbContext>();
    ...
    private async Task<TimeSlot> GetTimeSlotAsync(Guid slotId, CancellationToken ct)
    {
        await using var scope = scopeFactory.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<AppDbContext>();
    ...
    private InlineKeyboardMarkup BranchKeyboard(IList<Branch> branches, string prefix) =>
        new(branches
            .Select(branch => new[]
            {
    ...
    private InlineKeyboardMarkup ServiceKeyboard(IList<Service> services, string prefix) =>
        new(services
            .Select(service => new[]
            {
    ...
    private InlineKeyboardMarkup SlotKeyboard(IList<TimeSlot> slots) =>
        new(slots
            .OrderBy(SlotScheduler.NextOccurrence)
            .Select(slot => new[]
            {
    ...
    private static string BuildAppointmentConfirmationMessage(string branchName, string serviceName, TimeSlot slot, Ticket ticket) =>
        $"""
        ✅ Appointment booked.
        Ticket: {ticket.TicketNumber}
        Branch: {branchName}
    ...
    /// <summary>
    /// Sends a proactive notification to a customer's Telegram chat.
    /// Called by the queue notification service when a ticket is called.
    /// </summary>
